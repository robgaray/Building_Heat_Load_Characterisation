RG_Repair_multiple_h_double_side <-function (DF_input,STRING_date,VEC_vars, VEC_repair, nh_pre, nh_post)
{
  ###
  ...
  {
    current_post1 <-filter(Dat$IS_Repair_Pending, c(rep(0,nh_post),1), sides=1)
    current_pre1  <-filter(Dat$IS_Repair_Pending, c(1,rep(0,nh_pre)), sides=2)
  ...
  ### Date marks
  {
    Dat$Date_currweek_prev_hour <- Dat$DATE-nh_pre*60*60
    Dat$Date_currweek_post_hour <- Dat$DATE+nh_post*60*60
    
    Dat$Date_prevweek           <- Dat$DATE-1*60*60*24*7
    Dat$Date_prevweek_prev_hour <- Dat$Date_prevweek-nh_pre*60*60
    Dat$Date_prevweek_post_hour <- Dat$Date_prevweek+nh_post*60*60
    
    Dat$Date_postweek           <- Dat$DATE+1*60*60*24*7
    Dat$Date_postweek_prev_hour <- Dat$Date_postweek-nh_pre*60*60
    Dat$Date_postweek_post_hour <- Dat$Date_postweek+nh_post*60*60  
  }
